...
def get_json(options):
    ...
    IF = '/tmp/.IS_UNKNOWN_ERROR_MSG.log'
    IS_UNKNOWN_ERROR_MSG = ('UNKNOWN: The plugin' in str(ret) and 'requested does not exist' in str(ret))
    if IS_UNKNOWN_ERROR_MSG:
        def L(dat):
          with open(IF,'a') as f:
            f.write(str(dat)+"\n")
        L(json.dumps(list(os.environ.keys())))
        if 'NAGIOS__SERVICEMETA_PLUGIN_UPDATE_SERVICE_DESCRIPTION' in os.environ.keys() and \
          'NAGIOS_HOSTNAME' in os.environ.keys() and \
          'NAGIOS__SERVICEMETA_PLUGIN_FILE_NAME' in os.environ.keys():
              pynag_execute_cmd = ['pynag','execute',str(os.environ.get('NAGIOS_HOSTNAME')),'{}'.format(str(os.environ.get('NAGIOS__SERVICEMETA_PLUGIN_UPDATE_SERVICE_DESCRIPTION')))]
              L(pynag_execute_cmd)
              try:
                output = subprocess.check_output(pynag_execute_cmd, stderr=subprocess.STDOUT, shell=False, timeout=3,universal_newlines=True)
              except subprocess.CalledProcessError as exc:
                L("Status : FAIL", exc.returncode, exc.output)
              else:
                L("OK- Subprocess Output: \n{}\n".format(output))

#    print('IS_UNKNOWN_ERROR_MSG={}'.format(IS_UNKNOWN_ERROR_MSG))
...
def run_check(info_json):
    ...
    if 'stdout' in info_json and 'returncode' in info_json:
        ...
        err = "{} {}".format("xxxxx", info_json['error'])
        err = info_json['error']
...
def main():
    ...
    try:
        ...
        if options.debug:
            ...
            err_msg = "{} {}".format("XXXX", e.error_message)
